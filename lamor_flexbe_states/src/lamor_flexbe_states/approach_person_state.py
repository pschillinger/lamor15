...
from stalking.msg import StalkPose
...
class ApproachPersonState(EventState):
	...
	def __init__(self):
		...
		super(ApproachPersonActionState, self).__init__(outcomes = ['goal_reached', 'goal_failed','command_error'], input_keys = ['person_pose'])
		...
		self._client = ProxyActionClient({self._topic: StalkPose}) # pass required clients as dict (topic: type)
